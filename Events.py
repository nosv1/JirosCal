...
class Event:
    ...
    def to_embed(self):
        ...
        embed.add_field(
            name=Support.emojis.space_char,
            value=f"{Support.emojis.calendar_emoji} Upcoming Races **|** {Support.emojis.loudspeaker_emoji} Follow Host **|** {Support.emojis.bell_emoji} Set Reminders",
            inline=False
        )
...
async def send_calendar(client, message, user, days_span=28):
    ...
    if not jc_guild: # if not in db, create new one
        jc_guild = Guilds.Guild(message.guild.id, name=message.guild.name, prefix=f"@{Support.get_jc_from_channel(message.channel)}")
        jc_guild.edit_guild()
        ...
        while True or event and event.edited: 
            ...
            if not event:
                ...
                while not editing:
                    ...
                    embed.description += f"```{', '.join(str(bw) for bw in event.break_weeks) if event.break_weeks != [] else event.break_weeks} ```"
